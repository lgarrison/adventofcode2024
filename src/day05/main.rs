...
fn parse_p1(txt: &str) -> (Vec<Rule>, Vec<UpdateP1>) {
    let mut t = txt.split("\n\n");
    (parse_rules(t.next().unwrap()), UpdateP1::from_str(t.next().unwrap()))
}

fn sort_by_rules(update: &Update, rules: &RulesMap) -> Update {
    let mut new = update.clone();
    new.sort_by(|a,b|
        rules.get(a).and_then(|r|
            r.contains(b).then_some(Ordering::Less)
...
fn part2(txt: &str) -> isize {
    ...
    let unordered: Vec<Update> = updates_p1.iter().filter_map(|updatep1| 
        if !updatep1.check_rules(&rules) { Some(updatep1.vec.clone()) } else { None }
    ).collect();
    // println!("{:?}", unordered);
    let rules_map: RulesMap = get_rules_map(&rules);
    // println!("{:?}", rules_map);
    let ordered: Vec<Update> = unordered.iter().map(|update| sort_by_rules(&update, &rules_map)).collect();
    // println!("{:?}", ordered);
